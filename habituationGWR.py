...
starting_weight = 4    # stv

# TODO: set up an input pulse called pls
pulse = [0, 0, 1, 0, 0]     # pls = 

# TODO: then create a list of 6 pulses, called x, to use for input
input_stimulation = pulse * 6            # x = 

weight = starting_weight     # v = stv # Set connection weight to start weight value

habituation = 0.7 # rate at which the sea slug gets used to the pulse

###############################
# Set up and run simulation
###############################

nTs = len(input_stimulation) # find the length of the input list
...
for t in range(nTs):
    output[0,t] = input_stimulation[t]*weight
    
    if input_stimulation[t] > 0:
    # when the sea slug habituates, set:
        weight *= habituation
...
showresults(input_stimulation,output,nTs,starting_weight)
